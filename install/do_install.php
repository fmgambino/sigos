...
if (!file_exists($settings_file)) {
    die("Arquivo de configuración no encontrado!");
} else {
...
if (!empty($_POST)) {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        echo json_encode(["success" => false, "message" => "Por favor ingrese un email valido."]);
        exit();
    }
    ...
    if (!$is_installed) {
        echo json_encode(["success" => false, "message" => "¡Parece que esta aplicación ya está instalada! No puede volver a instalarlo."]);
        exit();
    }
